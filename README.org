...
- Build JSON documents programmatically via quasiquote.
...
If everything installed successfully you should be up and running:
    ...
    : scheme@(guile-user)> (scm->json (list 1 2 3))
    : [1, 2, 3]
...
** Examples using quasiquote

It is also possible to build JSON documents programmatically using the
quasiquote feature of scheme. Here are some examples:
...
- Build the object { "project" : "foo", "author" : "bar" }:
    ...
    : scheme@(guile-user)> (scm->json `(("project" . "foo")
                                        ("author" . "bar")))
