...
- [[http://download.savannah.gnu.org/releases/guile-json/guile-json-2.0.0.tar.gz][guile-json-2.0.0.tar.gz]]
...
- Build the object { "project" : "foo", "author" : "bar" } using a hash table
  (needs (ice-9 hash-table) module). See how symbols can also be used:

    : > (scm->json (alist->hash-table '((project . foo) (author . bar))))
    : {"project":"foo","author":"bar"}

- Build the object { "values" : [ 234, 98.56 ] }:

    : > (scm->json (alist->hash-table '(("values" 234 98.56))))
    : {"values":[234,98.56]}
...
- Build the object { "values" : [ 234, 98.56 ] } again, this time using
  a variable:
    ...
    : > (scm->json (alist->hash-table `(("values" ,@values))))
    : {"values":[234,98.56]}
