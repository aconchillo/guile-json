...
- [[http://download.savannah.gnu.org/releases/guile-json/guile-json-3.2.0.tar.gz][guile-json-3.2.0.tar.gz]]
...
Also, the /json-invalid/ exception is also thrown (without arguments) if an
invalid scheme type is given when building a JSON document.
...
- Parse the [1, 2, , , 5] array. Note how empty slots are filled with
  /*unspecified*/ (use /(unspecified? value)/ to check for /*unspecified*/):

    : > (json-string->scm "[1, 2, , , 5]")
    : #(0 1 2 #<unspecified> #<unspecified> 5)
